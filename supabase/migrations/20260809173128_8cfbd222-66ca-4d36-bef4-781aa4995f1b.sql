-- Pin search_path on internal email queue helpers (all references are schema-qualified)
...
-- Internal queue plumbing must not be callable through the public Data API
...
-- Role checks stay callable by signed-in users only (never anon)
